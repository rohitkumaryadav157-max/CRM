...
function Manviewenq() {
 ...
 const [rem,setRem] = useState('');
  ...
  const [uid, setUid] = useState('');
   ...
   //code for showing data to enquiry page according to manager login at particular loction 
  const getenq = async () => {
    const response = await axios.get("http://localhost:3000/api/enq");
    const response2 = await axios.get(`http://localhost:3000/api/user/${localStorage.getItem('manager')}`) //
    console.log(response);
    if (response.data.msg == "success" && response2.data.msg =="success") {
      setMydata(response2.data.user) //filter enquiry on the basic of manager login 
